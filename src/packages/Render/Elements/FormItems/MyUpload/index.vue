    ...
    <el-table v-if="state.fileListData.length>0" v-loading="state.loading" :data="state.fileListData" row-key="id" 
      ...
      <el-table-column v-if="!props.disabled" label="操作" fixed="right" header-align="center" align="center" width="35">
        <template #default="{ row,$index }">
          ...
          <el-button v-auth="'api:admin:file:delete'" icon="Delete" size="small" text type="danger" @click="onDelete(row,$index)"></el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-button v-if="!props.disabled" v-auth="'api:admin:file:upload-file'" type="primary" icon="Upload" @click="onUpload"> 上传 </el-button>
...
const state = reactive({
  loading: false,
  fileFormTitle: '', 
  fileListData:JSON.parse(JSON.stringify(props.modelValue))||[] as Array<any>,
  fileLogsTitle: '',
