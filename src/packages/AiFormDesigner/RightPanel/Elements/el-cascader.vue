...
import TreeNodeDialog from "../TreeNodeDialog.vue";
 const   props=defineProps( ["data"])
  ...
  const  remove=(node, data)=> {
      ...
      const index = children.findIndex(d =>(!!d.id&&d.id===data.id)||d.value === data.value);
       ...
       return   h('div',{class:'custom-tree-node'},
         [
          ...
          h('span',{class:'node-operation'},
            [
              h(ElIcon,
               {onClick: () => append(data)},[h(resolveComponent('Plus'))]
               ),
              h(ElIcon,{onClick: () => remove(node,data)},[h(resolveComponent('Delete'))])
            ]
            )
          ])
// return(
