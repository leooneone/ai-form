        ...
        <el-tabs v-model="state.activeTabName" :stretch="true">
          ...
          >
            <div class="components-list">
              <template v-for="(com, idx) in item.coms">
                <div class="components-title">{{ com.title }}</div>
                <Draggable
...
///是否被外部使用
const isInUsed = inject("isInUsed")||function(){return false}
  ...
  comTabs: [
    {
      ...
      name: "custom",
      coms: [
        { title: "工作流组件", model: configs.customComponents },
        { title: "组合型组件", model: configs.customGroupComponents },
      ],
    },
    {
      label: "基础组件",
      name: "common",
      coms: [
        { title: "输入型组件", model: configs.inputComponents },
        { title: "选择型组件", model: configs.selectComponents },
        { title: "布局型组件", model: configs.layoutComponents },
        { title: "扩展组件", model: configs.extComponents },
      ],
    },
  ],
  isErrorShow: false,
